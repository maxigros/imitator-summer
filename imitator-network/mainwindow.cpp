...
void MainWindow::on_pushButton_connection_start_clicked()
{
     ...
     * Функционал
     *************************************************************************/
    moduleNetwork = new BoxNetwork(configWriterMode);
    threadNetwork = new QThread;
    moduleNetwork->moveToThread(threadNetwork);
    connect(threadNetwork, SIGNAL(started()), moduleNetwork, SLOT(process()));
    connect(moduleNetwork, SIGNAL(finish()), threadNetwork, SLOT(quit()));
    connect(threadNetwork, SIGNAL(finished()), moduleNetwork, SLOT(deleteLater()));
...
void MainWindow::on_pushButton_generation_start_clicked()
{
     ...
     * Функционал
     *************************************************************************/
    moduleGenerate = new BoxGenerate(configFilename);
    threadGenerate = new QThread;
    moduleGenerate->moveToThread(threadGenerate);
    connect(threadGenerate, SIGNAL(started()), moduleGenerate, SLOT(process()));
    connect(moduleGenerate, SIGNAL(finish()), threadGenerate, SLOT(quit()));
    connect(threadGenerate, SIGNAL(finished()), moduleGenerate, SLOT(deleteLater()));
    connect(threadGenerate, SIGNAL(finished()), threadGenerate, SLOT(deleteLater()));
    // Обмен данными между BoxNetwork и BoxGenerate
    connect(moduleGenerate, SIGNAL(sendConfigParams(QMap<QString,double>)),
            moduleNetwork, SLOT(getConfigParams(QMap<QString,double>)), Qt::DirectConnection);
    // ToDo: connect для sendData
